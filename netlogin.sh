...
function check_connection() {
    ...
    local status_page
    status_page=$(curl -fsS --connect-timeout 8 --max-time 15 "https://${portal_host}/") || status_page=""
...
load_portal_config() {
    ...
    portal_config=$(curl -fsS --connect-timeout 8 --max-time 15 -A "$user_agent" -G "$config_url" \
...
function logout() {
    ...
    logoutResult=$(curl -fsS --connect-timeout 8 --max-time 15 -A "$user_agent" -G "${portal_api}/logout" \
...
function connect() {
    ...
    authResponse=$(curl -sS --connect-timeout 8 --max-time 15 -A "$user_agent" -w '\n%{http_code}' -G "${portal_api}/login" \
